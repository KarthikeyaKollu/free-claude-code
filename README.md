...
`fcc-claude` reads the current configured port and auth token each time it starts, sets the Claude Code environment variables (including a 1M-token `CLAUDE_CODE_AUTO_COMPACT_WINDOW` for auto-compaction), and then launches the real `claude` command. When proxy auth is disabled, it still passes `ANTHROPIC_AUTH_TOKEN=fcc-no-auth` so newer Claude Code versions do not stop at their local login gate before contacting the proxy.
...
- `fcc-claude`: launches Claude Code with the configured local proxy URL, an auth-token env var or `fcc-no-auth` sentinel, model discovery flag, and a 1M `CLAUDE_CODE_AUTO_COMPACT_WINDOW` for auto-compaction.
